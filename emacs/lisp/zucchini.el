...
(defvar zucchini-current-buffer nil)
(defvar zucchini-local-directory nil)
(defvar zucchini-script-name nil)
...
(defun zucchini--task-wrapper (body)
  `(lambda ()
     ...
     (unwind-protect
         (progn ,@body)
       (with-current-buffer zucchini-current-buffer
...
(defun zucchini-play ()
  ...
  (setq zucchini-project-directory
        (locate-dominating-file "." "zucchini-private.el"))
  (if zucchini-project-directory
      (setq zucchini-script-name "zucchini-private.el")
    (setq zucchini-project-directory
          (locate-dominating-file "." "zucchini.el"))
    (setq zucchini-script-name "zucchini.el"))
  (setq zucchini-current-buffer (current-buffer))
  (setq zucchini-local-directory default-directory)
  (if (not zucchini-project-directory)
      (message "Can't find Zucchini script")
    (when (zucchini--load
           (file-name-concat
            zucchini-project-directory zucchini-script-name))
      (zucchini--press))))
